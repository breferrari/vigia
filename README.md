...
The sparklines are change density over time. The bars locate change within each file. The list is pinned, so those signals stay on screen while you read the diff under them; it grows to the number of changed files, caps, and scrolls with `J` and `K` when there are more than fit. The scrollbar beside the diff is file-granular plus where you are inside the current file, which is everything it can know without reading every file's length. The frame time sits in the status bar because it is a promise, not a diagnostic: it is the p99 of the last 128 frames, against the 16ms this is gated at. The memory beside it is read once a frame and costs 193ns to read.
