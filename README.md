...
The list is **pinned**, so the signals stay on screen while you read the diff under them. Past 134 columns it moves *beside* the diff instead, as a left rail, so a path sits against its own numbers rather than across a void that grows with the pane. Every file gets the same row in both regions:
