...
| `m` `Esc` | **every view setting in one box**, the ones above and the two the config file sets, with `↑` `↓` to move and `Space` to flip. `remember between runs` keeps what you flip for next time, and `reset to defaults` puts every row back. `Esc` puts it away |
