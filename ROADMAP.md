...
| ⬜ | The memory readout has no Windows path | [#56](https://github.com/breferrari/vigia/issues/56) |
...
| The memory readout has no Windows path ([#56](https://github.com/breferrari/vigia/issues/56)) | #41, 2026-07-31 | Phase 5 | `tasklist` is the only dependency-free reader on Windows and it is a process spawn: **42.8ms median** over twelve runs on the reference machine, against a 16ms frame budget, so it is 2.7x the budget for the read alone. No sampling interval rescues it, since the frame that pays it is still a hitch and a cached value is stale by an unbounded amount. `GetProcessMemoryInfo` would cost 1% of that and needs `windows-sys`, which unlike `libc` on macOS is **a new crate in the graph** rather than a new edge to one already compiled, so it is a spec decision rather than an implementation detail. Shipping two platforms out of three was chosen deliberately over shipping none, which is the opposite of the call [#16](https://github.com/breferrari/vigia/issues/16) and [#24](https://github.com/breferrari/vigia/issues/24) made, and the difference is that those two are *guarantees* while this is a readout: a missing cell is visibly missing, where a guarantee that silently holds on two platforms is not |
