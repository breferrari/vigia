...
|---|---|---|---|
| A row's wrap and its frame are sized by two expressions that disagree by two columns ([#474](https://github.com/breferrari/vigia/issues/474)) | #466, 2026-09-08 | Shelf | Found measuring the enclosure's width rung. `View::layout` wraps at `content_width(gutter, width)` and `Renderer::note_row` draws at `glyphs.width - line_origin(gutter)`, and below some width the second runs two columns past the first: at fifteen columns a body wraps at seven inside a box whose inner width is nine. `content` is what every content line wraps at and not only a note's, so moving either expression moves the wrap of every line at narrow widths and needs the wrap suite around it, which is a width contract beside #466's one column. It is also why the enclosure's rung has no gate on its wide side, and the slack is measured: shaving `edge_width` by one column or by two leaves every gate green, and only the third cuts the drawn edge, so the boundary can sit two columns narrower than its own edge needs before anything can see it, and any distance wider with nothing to see it at all |
