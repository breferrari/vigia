...
**Measured, release, the same 100 x 500 fixture the budget gates use, fifteen minutes:** 18,262 frames, every one a full screen, from 17,527 write rounds and 3,506 files created and deleted. **RSS drift 0.17% against the 5% budget**, 25.3 MiB after warmup against 25.4 MiB at the end. At most 65 diffs held against 106 changed files, over 3,606 distinct paths ever changed, so the cache followed the diff rather than the session by a factor of fifty-five. Zero temp files retained. Three frames of 18,262 failed on a file deleted under the walk, which is the workload doing its job rather than a defect.

**What has not run is the window in the budget.** Fifteen minutes is what a session can measure; four hours is what CI will, nightly on Linux and weekly on all three targets; 24 hours needs a runner without the cap. The gate exists and fires, and the number it is standing on is a quarter-hour, which `SPEC.md` §10 records as open rather than closed.

**The run also settled where the warmup ends, and that is why the drift gate refuses short windows.** RSS climbs from 18.2 to about 25 MiB in the first thirty seconds and is flat for the remaining fourteen and a half minutes. The same code over a fifteen-second window reports **10% drift** and is measuring nothing but that climb. So the gate asserts only at ten minutes and above, and prints its numbers with a note below that. `SPEC.md` §7 carries the rule it leaves behind: a drift gate over a window shorter than its own warmup is measuring warmup, and refusing to answer is the point rather than the fallback.
