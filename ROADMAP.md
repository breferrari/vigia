...
**The same shape turned up twice more, in elements this issue does not touch, and both went to [#93](https://github.com/breferrari/vigia/issues/93).** The line-number gutter is measured against the region rather than the pane, so a diff crossing the pane height takes the gutter off every content row at 29 and 30 columns. And the heat strip, the rule and the scrollbar each traded a clipping writer for a direct cell write to save an allocation, and traded the clipping away with it; the strip's instance arrived in this issue's own `simplify` commit, which makes it a regression rather than a latent gap. Both were found by adversarial passes here and split back out, because they are this issue's *ruling* applied to other elements rather than this issue's *scope*.
