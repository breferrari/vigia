...
| ✅ | The Homebrew tap push is denied and nothing checks the token can write | [#137](https://github.com/breferrari/vigia/issues/137) |
...
**Filter: does it carry the reader's words to the agent, pinned to a line of the diff, without the pane stopping being a monitor?** Ruled first by the reader on 2026-09-04.
...
|---|---|---|---|
| A visible change that names no key is filed as internal, and 0.48.0's notes lost one ([#523](https://github.com/breferrari/vigia/issues/523)) | 0.48.0, 2026-09-11 | Shelf, taken same day | Found by reading 0.48.0's published notes after the release ran green. `changelog-entry.sh` dropped `A list behind the token puts the pane at any commit` on the word `token`, which the list carried for the release's own credentials and which the pane now draws in its header, and the allow pass that exists for this asks for a backticked key the subject has none of. The notes and the changelog are corrected by hand. Second occurrence of [#467](https://github.com/breferrari/vigia/issues/467)'s class with a different word, so the fix widens the allow pass rather than tuning the drop list again: `the pane` survives it, measured over every first-parent subject to 0.48.0 at two visible changes recovered and no internal subject let through by that entry. |
| A subject the notes filter drops is reported only as a workflow notice ([#524](https://github.com/breferrari/vigia/issues/524)) | #523, 2026-09-11 | Shelf | Split out of #523 and the part that let it ship. The script already names every subject it drops, and its own comment says why: a drop is invisible everywhere else. A `::notice::` on a release run that went green is read by nobody, so the remedy does not reach. Shelved rather than taken there because the shape that would reach a person puts the drops in the release's published text, which is a ruling about what a reader sees. |
...
| A release that removes a key can be filed as internal, and 0.42.0 was ([#467](https://github.com/breferrari/vigia/issues/467)) | #457, 2026-09-08 | Shelf, taken | Found by reading 0.42.0's published notes after the release ran green. `changelog-entry.sh` drops any subject matching `rulings?`, and `CLAUDE.md` requires a revoked ruling to be deleted in the same commit as the change it governed, so the two always share a subject. The notes and the changelog are corrected by hand; the filter is instrument work and shelved for that reason. |
| `SHEET_KEEP` names two things, so removing a gesture reassigns the never-drop set by rank ([#464](https://github.com/breferrari/vigia/issues/464)) | #457, 2026-09-08 | Shelf | Found by #457's altitude reviewer and **older than that PR**: the double duty is stated in the constant's own comment and predates the masthead's removal. Shelved rather than taken there because splitting it moves the narrow-pane floor, which `SPEC.md` §11.1 pins by measurement, and #457's gate did go red on the reassignment rather than hiding it. |
...
| A release that removes a key can be filed as internal ([#467](https://github.com/breferrari/vigia/issues/467)) | Out of the Shelf, 2026-09-09 | Shelved the day it was found, as instrument work with no product pass blocked by it. Taken because the deferral's own reason was measured and did not survive it: the word that emptied 0.42.0 has emptied three release ranges in fifty-five, two of the three wrongly, and a fourth release lost a key's line out of a section that was never empty. A filter nobody can see failing is not waiting for a pass to be blocked by it. |
