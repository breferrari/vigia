...
**And the gutter was the same defect a third time, on the rows a reader actually reads.** The line-number width was measured against the region, and the stream's region loses two columns to a scrollbar, and that bar appears when the diff outgrows the pane. So at 29 and 30 columns a diff crossing the pane height took the whole gutter off every content row: not a reflow but an all-or-nothing disappearance, on the tick an agent's edit made the diff long. Found by the second adversarial round, after `planning_width` had already fixed the two regions above it, which is what a third instance of one shape is worth recording for.

**One panic was a simplification's bill, arriving three commits later.** The heat strip used to be drawn through `set_stringn`, which clips; it was traded for a direct cell write to stop an allocation per cell, and the clipping went with it unremarked. `render` promises any area is legal, so an area wider than its buffer then aborted the whole paint on the one row carrying a strip. The rule and the scrollbar had the same trade made earlier and the same hole; all three take the clipping form now.
...
**The footer's colours shipped in the same PR, and that was a scope deviation rather than part of [#77](https://github.com/breferrari/vigia/issues/77).** `assets/preview.svg` draws the frame and memory readouts in its cyan and the follow marker in green; the shell drew the whole right-hand side in one grey, so the two numbers a reader *checks* and the marker saying the mode is on looked like the words a reader *reads*. It is the same §5.1 audit that generated every other row in this phase, found while reading the picture against the shell for the glance row, and it is one region down rather than the one #77 rules. #77's own plan had put the mockup's colours out of scope, so this is recorded as taken deliberately and against that plan rather than absorbed: the alternative was filing an issue for work already written, tested and ruled in the spec. The row's own counters are still one dim grey where the picture draws them green and red, so the colour half of that departure stands.

Seventeen gates, none of which the old suite could have failed: no fixture varied counts width across rows, none scrolled a list between two windows, none renamed a file at a width where the pair does not fit, nothing asserted a counts cell against the boundary of every unit, no test read a style off the footer at all, and the only panic sweep drew one benign fixture at seven sizes. One of the seventeen catches a hang rather than a wrong pixel: the recolouring pass asked whether a cell *opened* a measurement and whether it *carried* one, and `>` answers yes and no, so `>1s` spun the two walks against each other forever with the pane frozen. Reachable from the ordinary path, since that is simply what a frame over a second draws.
