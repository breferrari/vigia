...
| ✅ | The pre-flight reads a truncated board, and comparison 1 calls it drift | [#369](https://github.com/breferrari/vigia/issues/369) |
| ⬜ | The pre-flight spends 19 seconds, and the cheap-looking loop is the expensive one | [#371](https://github.com/breferrari/vigia/issues/371) |
...
| The pre-flight reads a truncated board, and comparison 1 calls it drift ([#369](https://github.com/breferrari/vigia/issues/369)) | #177, 2026-08-29 | **taken in #177's pass** | An instrument finding that would normally shelve, taken inside the product pass **because that pass was blocked by it**: the pre-flight is the gate step 1 clears before taking anything, and at a limit of 200 against 202 issues it dropped #2 and reported I2a as tracked by nothing. |
| The pre-flight spends 19 seconds, and the cheap-looking loop is the expensive one ([#371](https://github.com/breferrari/vigia/issues/371)) | #369, 2026-08-29 | Shelf | Instrument work and nothing was blocked by it, so it is filed with its attribution rather than folded in. Found by #369's `/simplify` round, which predicted the wrong loop: raising the fetch limit removes comparison 7's ceiling, and comparison 7 is **2.57s** of a **19.4s** run while comparison 3, which no fetch size bounds, is **13.34s**. Fewer rows, five processes each instead of one. |
