...
**[#77](https://github.com/breferrari/vigia/issues/77) is the same shape one row down, and it was the picture that had been right the whole time.** Every file row in `assets/preview.svg` puts the same element at the same `x`; `Painter::file_row` right-packed, so each element's position was a function of the widths of the ones outside it and no two rows agreed. The ladder runs once per region now and every row draws into the slots it produced. Two things a first attempt got wrong are worth more than the fix: a column reserved where **no** drawn row can fill it spends I6's columns on a signal that cannot appear, which at forty columns cost a path most of its name; and the pulse cannot be a column at all, because it lasts one tick and a slot for it would reflow every row on the tick a file was written. The picture settles the second one by drawing no pulse on a summary row, only on the diff heading. Three gates, none of which the old suite could have failed, because no fixture varied counts width across rows.
