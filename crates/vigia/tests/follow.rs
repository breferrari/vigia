...
fn top_file(app: &mut App, frame: &mut Frame) -> String {
    let view = app.view(frame, body()).expect("view");
...
fn a_change_moves_the_view_to_the_changed_file_with_no_input_at_all() {
    ...
    assert_eq!(top_file(&mut app, &mut frame), first);

    app.follow(&target, &frame);

    assert_eq!(
        top_file(&mut app, &mut frame),
        target,
...
fn scrolling_disengages_follow_and_the_next_change_does_not_move_the_view() {
    ...
    app.follow(&target, &frame);
    assert_eq!(top_file(&mut app, &mut frame), target);
...
fn a_position_survives_the_file_it_points_at_being_committed() {
    ...
    assert_eq!(top_file(&mut app, &mut frame), last);
