...
enum Reach {
    ...
    Both,
    /// Only reachable while a bounded mode is up, where the mode's own router
    /// answers the event before `action_for` ever sees it, and the surface writes
    /// its keys on its own edge. Carries why, for [`Reach::Shell`]'s reason.
    Mode(&'static str),
...
fn reach_of(action: &Action) -> Reach {
    match action {
        ...
        // `o`, and the sheet's own row for it. No mouse gesture reaches it.
        Action::ToggleOverview => Reach::Keyboard,
        ...
        // `m`, and the sheet's own row for it.
        Action::ToggleMenu => Reach::Keyboard,
        // The rest of `SPEC.md` §11.2 B22's mode. `menu::menu_route` answers the
        // arrows, `Space` and a press before `action_for` is reached, and the box
        // writes those keys along its own bottom edge, so the sheet teaching them
        // a second time would spend rows B13's ladder is already short of.
        Action::CloseMenu | Action::MenuMove(_) | Action::MenuFlip | Action::MenuRow(_) => {
            Reach::Mode("the config menu's own, named on its bottom edge")
        }
...
fn mouse_phrases() -> Vec<&'static str> {
    ...
    for hovered in [
        ...
        phrases.push(match hovered {
            ...
            // same reading, so the variants share it rather than each naming a
            // row of its own.
            Hovered::Button(..)
            | Hovered::Track(_)
            | Hovered::Row(_)
...
const GESTURES: [&str; 31] = [
    ...
    "left rail",
    "one file",
    // §11.1's list-alone row, after `s` because that is where the reader's own
    // order puts it: `s` narrows the diff and this one removes it. The token is the
    // half both spellings share, which is this array's rule and not this row's.
    "list alone",
    // B17's row, between `s` and `?` because that is where the reader's own
    // order puts it.
    "staged changes",
    // §11.1's position, beside `a` because both change what the frame walks
    // rather than only what is drawn from it.
    "branch point",
    // B19's row, after `a` because that is where the reader's own order puts it.
    "wrap",
    // B21's two rows, which are the `notes` group.
    "note rows",
    "send the note",
    // B22's door, between the notes group and the sheet's own row because that is
    // where the reader's order puts it: the two ways out of the pane sit together.
    "config menu",
    "this sheet",
    "quit",
    "wheel",
    ...
    "drag the diff",
    // B21's press, whose keys cell spells the target wide and the mark tight, so
    // the verb is the half that keeps one spelling.
    "open a note",
    // B21's second press, whose keys cell spells the target wide and short, so the
    // verb is again the half that keeps one spelling.
    "take it back",
...
/// the height at which the tallest rung fits. Every keyboard row added moves
/// that arrival up by one, and the ceiling keeps six rows of room over it.
const LADDER_HEIGHTS: std::ops::RangeInclusive<u16> = 6..=51;
...
fn the_sheet_spends_width_before_it_spends_gestures() {
    ...
    sweep!("sheet-width", |paint| {
        let short_and_wide = Rect::new(0, 0, 120, 25);
...
const SECTIONS: [&str; 6] = ["moving", "files", "view", "notes", "mouse", "leaving"];
...
enum RoomyRow {
    ...
    Air,
    /// What the pane is, in the row this rung leaves blank on its own.
    Said,
...
fn roomy_shape() -> Vec<RoomyRow> {
    let mut rows = vec![RoomyRow::Said];
    for (label, tokens) in [
        ("moving", &GESTURES[0..3]),
        ("files", &GESTURES[3..7]),
        // Seven: `r`, `s`, `o`, `a`, `b` and `w` join `f` in `view`, which is the
...
fn the_display_order_is_the_readers_and_the_narrow_floor_keeps_the_unguessable() {
    ...
    const NARROW: [(u16, &[&str]); 3] = [
        ...
        // after `DROP_ORDER` has given up eight, and every one of the three the
        // keep-set names is in it, plus the config menu's door, which outlives every
        // view toggle.
        (
            30,
            &[
...
fn the_two_column_rung_is_the_size_the_ruling_states() {
    ...
    let wide = Rect::new(0, 0, 120, 25);
    let tight = Rect::new(0, 0, 80, 25);

    sweep!("sheet-dimensions", |paint| {
        for (at, want, spelling) in [(wide, (104u16, 22u16), "wide"), (tight, (71, 22), "tight")] {
            let (buf, laid) = paint(at);
            let sheet = laid.sheet.expect("the pane draws no sheet at all");
            let (_, drawn) = read_sheet(&buf, &laid);
            assert!(
...
fn the_two_column_rung_arrives_at_the_width_the_ruling_states() {
    ...
    sweep!("sheet-arrival", |paint| {
        ...
        arrival = arrival_of(&mut paint, "keyboard", 70..=84, 25);
    });
    assert_eq!(
        arrival,
        Some(73),
...
fn the_two_column_rung_places_its_cells_where_the_plan_says() {
    ...
    for (w, h, cols, label, spelling) in [
        (120u16, 25u16, [2usize, 26, 56, 77], 56usize, "wide"),
        ...
        sweep!("sheet-columns", |paint| {
            ...
            // it names, so the space it opens with lands on the rule. Found rather
            // than indexed, because the line saying what the pane is is drawn only
            // while the table still fits beside it, so which row this is depends on
            // how many gestures the table holds.
            let at_heading = rows
                .iter()
                .position(|row| row.iter().collect::<String>().contains("keyboard "))
                .expect("the two-column rung draws a keyboard heading");
            let heading: String = rows[at_heading].iter().collect();
            assert_eq!(
                heading
                    .char_indices()
                    .nth(label)
            ...
            let row2: String = rows[at_heading + 1].iter().collect();
            let keyboard_first: String = rows[at_heading + 1][cols[0]..].iter().collect();
            let mouse_first: String = rows[at_heading + 1][cols[2]..].iter().collect();
            assert!(
            ...
            // pinned against.
            let heading_head: String = rows[at_heading][1..].iter().collect();
            assert!(
            ...
            for (n, token) in GESTURES.iter().enumerate() {
                ...
                let (col, row) = if n < split {
                    (cols[0], at_heading + 1 + n)
                } else {
                    (cols[2], at_heading + 1 + (n - split))
                };
...
fn the_height_ladder_pages_rather_than_dropping_and_fills_every_page_it_can() {
    ...
    let expected = [
        ...
        // row and names no gesture. `SPEC.md` §11.2 B22's row moved it from 24 to 25.
        (25, 19, 2),
        (26, 20, 2),
        (27, 21, 2),
        (28, 22, 2),
...
fn the_keys_cell_is_lit_and_the_verb_is_dim() {
    ...
    for (w, h, keys_at, verb_at, lead, spelling) in [
        // The two-column rung says what the pane is above its heading, so its
        ...
        // Non-vacuity below asserts the rung this case is named for.
        let row = sheet.top + lead;
        let (count, drawn) = read_sheet(&buf, &laid);
    ...
    // Which row they are on is found rather than counted: the line saying what the
    // pane is sits above them only while the table still fits beside it.
    let at = Rect::new(0, 0, 120, 25);
    let (buf, laid) = paint(&mut app, &mut frame, &mut highlighter, &history, at);
    let sheet = laid.sheet.expect("a pane that draws no sheet");
    let (_, headings) = read_sheet(&buf, &laid);
    let at_heading = u16::try_from(
        headings
            .lines()
            .position(|row| row.contains("keyboard "))
            .expect("the two-column rung draws a keyboard heading"),
    )
    .expect("a sheet taller than u16");
    let dim = theme
        .chrome_dim
        .fg
        .expect("the dim weight carries a colour");
    for (col, glyph, what) in [
        ...
        assert_eq!(
            buf[(sheet.left + col, sheet.top + at_heading)].symbol(),
            glyph,
            "{what} does not start at column {col}, so the weight read below is a \
             blank cell's"
        );
        assert_eq!(
            buf[(sheet.left + col, sheet.top + at_heading)].fg,
            dim,
...
fn the_one_column_rung_places_its_cells_where_the_plan_says() {
    ...
    for (w, h, keys_at, verb_at, first_key, mouse_from) in [
        ...
        (80u16, WHOLE_TABLE, 2usize, 26usize, 'j', Some(20usize)),
        (120, WHOLE_TABLE, 2, 26, 'j', Some(20)),
...
fn the_two_column_rung_swallows_what_lands_on_it() {
    ...
    let (_, laid) = paint(&mut app, &mut frame, &mut highlighter, &history, at);
    let sheet = laid.sheet.expect("a 120 by 24 pane draws no sheet");
    assert_eq!(
        sheet.width, 104,
...
fn the_roomy_rung_swallows_what_lands_on_it() {
    ...
    // A row no other rung reaches: the plain rung is thirty rows tall and the
    // two-column rung twenty, so row 33 of this sheet is the roomy rung's alone.
    let deep = sheet.top + 33;
...
fn the_counter_names_what_the_pane_reaches() {
    ...
    for (at, want) in [
        ...
        (Rect::new(0, 0, 32, 40), vec![counter("1-15")]),
        (Rect::new(0, 0, 30, 40), vec![counter("1-11")]),
    ] {
        let walked = walk_the_pages(&mut frame, &mut highlighter, &history, at);
...
fn a_resize_clamps_the_page_rather_than_closing_the_sheet() {
    ...
    // Four pages of nine rows, so page seven does not exist and page four is the
    // last.
...
/// What the sheet says the pane *is*, restated for [`TITLE`]'s reason. Two
/// spellings, because the narrowest rung that carries it has thirty-two columns
/// of room and the widest has a hundred and one.
const PURPOSE_WIDE: &str = "working tree's diff, live · it follows on its own";
const PURPOSE_TIGHT: &str = "working tree's diff, live";

#[test]
fn the_sheet_opens_on_what_the_pane_is() {
    // One pane of each rung that carries the line. The sheet answered *what can
    // I press*, which is only a question a reader who already knows the answer
    // would ask.
    sweep!("sheet-purpose", |paint| {
        for (at, rung) in [
            // A row taller than `area()`, which is the height at which the
            // two-column rung stops having the row for the line and keeps every
            // gesture instead.
            (Rect::new(0, 0, WIDE, TALL + 1), "two columns"),
            (ROOMY_PANE, "roomy"),
            ...
            let seen = match (sheet.contains("moving"), sheet.contains("keyboard")) {
                (true, _) => "roomy",
                (_, true) => "two columns",
                _ => "one column",
            };
            assert_eq!(
                seen, rung,
                "the {}x{} case draws the {seen} rung, not the {rung} one, so the \
                 drawer this case is named for was never reached:\n{sheet}",
                at.width, at.height
            );

            let said = rows
                .iter()
                .position(|row| row.contains(PURPOSE_WIDE))
                .unwrap_or_else(|| panic!("the {rung} rung never says what the pane is:\n{sheet}"));
            let first = rows
                .iter()
                .position(|row| GESTURES.iter().any(|g| row.contains(g)))
                .expect("the sheet draws no gesture at all");
            assert!(
                said < first,
                "the {rung} rung says what the pane is at row {said} and opens on a \
                 gesture at row {first}, so it still opens on the key table:\n{sheet}"
            );
            // Non-vacuity in the direction that matters: a line drawn over the
            // gestures rather than above them would satisfy the order above.
            assert_eq!(
                count,
                GESTURES.len(),
                "the line cost the {rung} rung a gesture, which is the one thing it \
                 may not do on a pane that holds them all:\n{sheet}"
            );
        }
    });
}
...
#[test]
fn the_line_fits_the_rung_that_carries_it() {
    // The failure this closes is a copy edit, and the shape of it is why the row
    // is found by its opening rather than by either spelling: the drawer clips at
    // the room it is given, so a spelling one column too wide is drawn short and
    // leaves the frame intact. A gate looking for a whole spelling would stop
    // finding the row rather than fail, which is how a mutation survived it.
    let (mut wide, mut tight) = (0usize, 0usize);
    walk_the_ladder("sheet-purpose-fit", |w, h, _, buf, laid| {
        let (_, sheet) = read_sheet(buf, laid);
        // Both spellings open with the tight one, so this finds the row whether it
        // was drawn whole, drawn short, or drawn at either spelling.
        let Some(row) = sheet.lines().find(|row| row.contains(PURPOSE_TIGHT)) else {
            return;
        };
        let said = row.trim_end_matches([' ', '│']);
        if said.ends_with(PURPOSE_WIDE) {
            wide += 1;
        } else {
            assert!(
                said.ends_with(PURPOSE_TIGHT),
                "at {w}x{h} the line is drawn short of either spelling, so a rung \
                 chose one its own box cannot hold:\n{sheet}"
            );
            tight += 1;
        }
        let drawn: Vec<char> = row.chars().collect();
        assert_eq!(
            (drawn.first(), drawn.last()),
            (Some(&'│'), Some(&'│')),
            "at {w}x{h} the line reached the sheet's own frame:\n{sheet}"
        );
    });
    assert!(
        wide > 0 && tight > 0,
        "the ladder drew the wide spelling {wide} times and the tight one {tight}, \
         so one of the two is never checked"
    );
}

#[test]
fn a_rung_that_gives_up_a_gesture_gives_up_the_line_first() {
    // The ladder's order, and the reason for it: a rung is narrow or short
    // because the pane is, and prose is the first thing such a pane cannot
    // afford. A rung gives a gesture up for width by dropping rows from the
    // table and for height by paging, and neither may happen with the line up.
    let (mut carried, mut rationed) = (0usize, 0usize);
    walk_the_ladder("sheet-purpose-drop", |w, h, _, buf, laid| {
        let (count, sheet) = read_sheet(buf, laid);
        ...
        if count < GESTURES.len() {
            ...
            assert!(
                !said,
                "at {w}x{h} the sheet draws {count} of {} gestures and spends rows \
                 saying what the pane is, so a pane short of room is being told what \
                 it is instead of what it can do:\n{sheet}",
                GESTURES.len()
            );
        } else if said {
            carried += 1;
    ...
    assert!(
        carried > 0 && rationed > 0,
        "the ladder saw {carried} panes carry the line and {rationed} ration \
         gestures, so one side of this is never exercised"
    );
}

#[test]
fn the_page_counter_still_counts_gestures_only() {
    // The line is not a gesture, so it may not move an ordinal. `Line::Heading`
    // is the precedent this rides on.
    sweep!("sheet-purpose-counter", |paint| {
        let at = Rect::new(0, 0, WIDE, 12);
        let (buf, laid) = paint(at);
        let (count, sheet) = read_sheet(&buf, &laid);
        let counter = counter_of(&sheet)
            .unwrap_or_else(|| panic!("a twelve row pane's sheet draws no counter:\n{sheet}"));
        assert!(
            counter.trim().starts_with('1'),
            "the first page does not begin at the first gesture: {counter:?}\n{sheet}"
        );
        assert!(
            counter.trim().ends_with(&GESTURES.len().to_string()),
            "the counter's total stopped being every gesture the tables hold: \
             {counter:?}\n{sheet}"
        );
        assert!(
            count > 0,
