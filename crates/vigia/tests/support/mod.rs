...
pub fn place_of(action: &Action) -> Place {
    match action {
        ...
        // Every gesture of the position list and the key that reads a place the
        // other way, for `b`'s reason one step out: where the pane stands is a
        // state, and a file that could open it somewhere else would make the thesis
        // a setting. The list is how a session says otherwise.
