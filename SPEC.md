...
**A row that has no such fact leaves the column blank rather than closing it.** This is the half that makes it universal rather than occasional: a sparkline does not exist until a file has been written once, so at launch *no* file has one, and a file with no line diff has no heat strip. Under right-packing each absence let that row's remaining elements slide into the space and pulled it out of line with its neighbours.
