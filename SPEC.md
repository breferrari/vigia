...
- [ ] **I3 is gated, and the window in its budget has not been run.** Measured on the reference machine, release, over the same 100-file, 100k-line fixture the budget gates use, 900 seconds: 18,262 frames, every one a full screen, from 17,527 write rounds and 3,506 files created and deleted. **RSS drift 0.17%** against the 5% budget, 25.3 MiB after warmup against 25.4 MiB at the end, peak 26.6 MiB, from a cold 18.2 MiB. At most 65 diffs held against 106 changed files, over 3,606 distinct paths ever changed. Zero temp files retained. Three frames of 18,262 failed on a file deleted under the walk, which is the workload doing its job.
  ...
  It also fixes where the warmup ends, which is the number §7's rule is written against: RSS reaches its plateau about **thirty seconds** in and is flat for the remaining fourteen and a half minutes, while the same code over a fifteen-second window reports **10% drift** and is measuring nothing but the climb.
