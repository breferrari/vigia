...
**The footer's right-hand side carries three colours, and they are the picture's.** `assets/preview.svg` draws `0.8ms` and `24MiB` in its cyan, the word `frame` beside them in the same grey as the hints, and the follow marker in green. The shipped footer drew all of it in one grey until 2026-08-02, so the two numbers a reader *checks* and the marker saying the mode is on looked like the words a reader *reads*. §5.1's rule that a published artifact answering a question is the answer applies to colour exactly as it applied to layout. The position `N/M` keeps the grey it has: the picture gives no colour for it, and it is a place rather than a measurement.
