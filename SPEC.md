...
**A region's file rows draw their glance elements in columns**, which is [#77](https://github.com/breferrari/vigia/issues/77) and which the picture had promised since before anything was built. Every element used to come out of a budget that had already lost the elements outside it, so its position was a function of *their* widths on *that* row, and the counts cell is the widest variable thing there. Three sparklines then failed to read as one small-multiples chart, which is the reading a list exists to give. So the ladder runs **once for a region** and every row draws into the slots it produced. Its order is unchanged: counts, then heat, then sparkline.
...
**The counts cell is two fixed halves of five columns**, `+9999` and `-9999`, with magnitude giving way to a shorter form past that rather than to more digits: `+15k`, `+1M`, `+4G`. That is the rule the status bar's frame and memory cells already follow with `>1s` and `>9GiB`. A field wide enough for `+4294967295` would spend eleven columns a row forever on a number no file reaches. The halves narrow to three columns together on a pane that cannot afford five.

**The layouts are written out as a table rather than allocated element by element**, and that is what makes narrowing monotone. Allocating greedily in priority order is what the rest of this document's ladders do, and for a *shared* layout it oscillates: measured across every width, the row lost its sparkline at 37 columns, got it back at 40 and lost both glance elements at 41, because each element took the widest rung it could afford and starved whatever came after. Widening a pane must never take something away. Each row of the table gives up exactly one thing against the row above it and gains nothing, so the whole ladder is monotone by construction rather than by argument. The drop order, narrowing: the pulse **label**, then the sparkline's resolution, then the strip's, then the counts' width, then the sparkline, then the strip, then the pulse **mark**, and the counts last, because they are the row's content rather than a signal drawn beside it. `crates/vigia/tests/legibility.rs::the_glance_columns_collapse_in_one_order` asserts the monotonicity as a rule and pins the walk with the width each state begins at, because a rule alone cannot see a boundary that moved.
