...
**Each region plans its own**, because the two are different widths: the list insets by the caret and may have given a column to a scrollbar. Neither reads a row, so a heading scrolling past cannot move the pinned region above it.
...
**There is no trailing reserve.** The picture leaves about a sixth of the row blank on its right, and that is an artifact of a fixed canvas at a width where nothing degrades rather than a promise; spending it at forty columns is what I6 forbids. The claim that *is* load bearing is the other one, and it holds: the columns are decided before the path is drawn and the path takes what is left, so scrolling a long path into view cannot move them.
