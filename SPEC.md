...
Detection is a precedence chain, first answer wins: `VIGIA_COLOR`, then `NO_COLOR`, then `TERM=dumb`, then `COLORTERM` claiming 24-bit, then `TERM` containing `256color`, then Windows with `WT_SESSION` at truecolour, then Windows at 256, then sixteen. **`TERM` only ever promotes**: on Windows it is usually unset and where Git Bash sets it to `xterm` that is a name from a world where sixteen was the floor, so reading it as a demotion would take 256 colours away from every Windows console, all of which have them. The floor is sixteen because it is the one answer that is never actively wrong.
