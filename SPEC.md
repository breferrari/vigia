...
**Each region measures its own**, over the rows it is about to draw. One set shared between the pinned list and the diff stream would let scrolling the *diff* move the *list's* columns, and the list is the map that has to hold still. Measuring instead over every changed file would make them stable under scrolling and is forbidden for [#49](https://github.com/breferrari/vigia/issues/49)'s reason exactly: it needs every changed file's diff, which puts the frame back in proportion to the size of the diff against I4. A region may measure what it has already paid to draw and nothing more.

**A column is reserved only where some drawn row would fill it**, and the alternative was tried and is worse. Reserving the sparkline's width in a region where nothing is tracked spends columns on a signal that cannot appear, and at forty columns it took `crates/vigia-core/src/frame.rs` down to a stub beside a blank gap. A column every row pays for has to be earned by at least one.

**The pulse is not a column**, and that is the one glance element ruled out of the set. It is per-row and lasts exactly one tick, so a slot reserved for it would reflow every row on the tick a file was written, which is the one moment anybody is looking; and a slot taken only by the rows that pulse is the right-packing this ruling removes. `assets/preview.svg` is the tiebreak and draws no pulse on a summary row at all, only on the diff heading. It comes out of the **path's** room instead, which is what the mark surviving narrowing already meant. At forty columns that is strictly better than what it replaced: the pulsing row used to spend fourteen columns on the label and draw neither heat nor sparkline, and now draws both beside the one-column mark.

**The ladder is ragged, and both reasons are the design rather than defects.** "The heat strip outranks the sparkline" means first refusal on the budget, not a veto over what is left: the strip's narrowest non-empty rung is six slices where the sparkline's is four buckets, so between those two widths the strip cannot fit and the sparkline can, and a row draws a sparkline with no strip beside it. And the counts cell outranks both, so the width at which it arrives *removes* a sparkline that had fitted without it. Neither is monotone and asserting that they were would be asserting something false, so `crates/vigia/tests/legibility.rs::the_glance_columns_collapse_in_one_order` pins the walk instead: nine states with the width each begins at. **The widths are pinned as well as the order**, because a sequence alone is a weak gate: removing the gap the heat strip reserves shifts every boundary below it by a column and leaves the walk identical, and that mutation survived until the widths went in.
