...
**The decline's first clause was false too.** DEC Locator's DECEFR reports a pointer leaving a filter rectangle, which is a mouse protocol reporting a pointer leaving. It is out of reach, since `crossterm` exposes no DEC Locator and a second mouse mechanism inside I8's takeover is not on offer, but an absolute about the outside world was asserted and never checked, and a quantifier is where the next one will hide.
...
**The one number that goes the other way.** Every feature added to this pane so far has cost the frame path something. A pin removes one of I4's two exceptions from the frames it is on: the diff's height is counted for every changed file once per tick and is the only one a frame with nothing anchored meets, and a pinned frame reads its total off the pinned file's span instead. The gate asserts the **unpinned** frame counted something before it asserts the pinned frame counted nothing, because a zero over a fixture that had nothing to count is not evidence, and that is the same two-fixture rule §7 states for every other cost claim here.
...
**The split reached five sites, four outside the walk**: `App`'s pinned `G`, its drag and its page steps subtracted a display height from a logical span, `view::landing_of` could judge a change visible below the fold, and the walk's own `short` read a display-full pane as short. Each measures in display rows now. The bar's far end stays out: it asks for a row past the end and lets the walk clamp, since the end of the diff is the walk's to place.
...
**`ratatui-textarea` 0.9.2 costs one crate in the lock file and six in the binary.** The same command on the `crossterm` feature alone resolves `ratatui-core`, `ratatui-crossterm`, `ratatui-widgets`, `unicode-segmentation` and `unicode-width`, every one already in the lock. **Corrected 2026-09-06 when the build landed ([#419](https://github.com/breferrari/vigia/issues/419))**: `cargo tree -p vigia` over the build graph gained six crates on Windows and seven on musl, `time` and its tree among them, because the crate declares `ratatui-widgets` with that crate's defaults and `calendar` is one, and the lock file lists optional dependencies whether or not anything enables them, which is what the count above saw. §6 carries the number that ships. The `tui-widgets` workspace was read and none of it taken: `tui-scrollview` renders the whole content into a buffer of its full size every frame, which is the shape I4 forbids; `tui-popup` floats where the ask is inline; `tui-prompts` is one line.
...
Ruled 2026-09-04, reader ([#374](https://github.com/breferrari/vigia/issues/374)); implemented 2026-09-10. `WRITTEN_LAYER_BUDGET` gave six documents a byte ceiling each, and every ceiling equalled its file's size on the commit that set it, so the rule in practice was that the written layer never grows. That is right for prose and wrong for a ledger, and the difference is what they are rather than how big they are.

**Prose keeps the ceiling.** `SPEC.md`, `RULINGS.md`, `CLAUDE.md` and the take-next skill say what holds and why; they should get shorter as rulings are replaced, and a ratchet is the instrument that makes a pass adding a paragraph go and find one to delete. Over 2026-09-03/04 it forced a dependency paragraph to be compressed, two sentences a change had made false to be deleted, a clause restored once bytes were free again, and two ceilings *lowered* when prose genuinely came out. Its value is not the CI failure; it is that a pass has to stop and ask whether a paragraph earns its place.

**Ledgers lose it.** `ROADMAP.md` and `REVOCATIONS.md` record events. A row exists because an issue was filed and an entry because a ruling was withdrawn, and neither can be declined to fit, so a ceiling there produces a mandatory raise with no decision in it: friction without judgement. `REVOCATIONS.md` is the worse case, and the reason is governance rather than size. It is the record of a session being overruled, so it is the one document whose growth is evidence the process is working, and the instrument deciding how much room that record gets was itself set by sessions: 1701, then 3217, then 3192, none of them by the reader.

**`WRITTEN_LAYER_TOTAL` is deleted, and one number is the whole argument.** Its stated job was that a ceiling may rise only while another falls by at least as much, so that a ruling moving house could be told from a ceiling edited to fit. Across six merges it moved by exactly the per-file delta every time: #400 at −9 and −9, #395 at +906 and +906, #399 at +600 and +600, #398 at +157 and +157. A sum check cannot fail when the sum is edited to match, and what caught the lockstep edit on [#402](https://github.com/breferrari/vigia/pull/402) was a reviewer reading the diff line rather than the arithmetic. It could never have worked while a ledger row forces the raise regardless. By the end its docblock had grown one paragraph per raise, nine of them, to the length `register.rs` caps a docblock at, which is the same accretion measured from the other side.

**The objection, recorded rather than waved away.** Without the total, a prose ceiling can be raised whenever it is convenient. That was already true, and visibility is the enforcement either way: a raise stays a visible, reviewed line in a diff. And the fact that most undercuts this ruling, recorded because it should be: `SPEC.md` grew 1,538 bytes over the same two days the ratchet was working. Every one of those bytes was argued for and paid for, and the alternative is unbounded, so the ceiling stays; but it is softer than it looks and that is the number which says so.

`crates/vigia/tests/package.rs::no_ledger_carries_a_byte_ceiling` is what stops a ledger being quietly returned to the budget: it names the two and fails with the reason rather than with a number.
