...
`WRITTEN_LAYER_BUDGET` and `LEDGERS` in `crates/vigia/tests/package.rs` carry the standing rule and the reason for it. What belongs here is the evidence it was kept and dropped on.

**What the ceiling bought over 2026-09-03/04**, which is why prose keeps it: a dependency paragraph compressed, two sentences a change had made false deleted, a clause restored once bytes were free again, and two ceilings *lowered* when prose genuinely came out. Its value is not the CI failure; it is that a pass has to stop and ask whether a paragraph earns its place.

**Why `REVOCATIONS.md` is the worse of the two ledgers**, and the reason is governance rather than size. It is the record of a session being overruled, so it is the one document whose growth is evidence the process is working, and the instrument deciding how much room that record gets was itself set by sessions: 1701, then 3217, then 3192, none of them by the reader.
